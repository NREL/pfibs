...
gamma = 4 # Penalty term to improve solver performance.
F_pc = inner(grad(v),Ke*grad(u))*dx \
    + inner(grad(d),De*grad(c))*dx \
    + c*d/dt*dx \
    - Constant(tp-gamma)/Fday*inner(Constant((2*Ke*RTF)*(1.0-tp)*(1+Ac))/c*grad(c),grad(d))*dx \
    - dot(ise_a*n,v*n)*ds(1) \
    - dot(ise_c*n,v*n)*ds(2) #\
    #- Constant(1-gamma)/Fday*dot(ise_a*n,d*n)*ds(1) \
    #- Constant(1-gamma)/Fday*dot(ise_c*n,d*n)*ds(2)
F_pc = action(F_pc, U_n)
J_pc = derivative(F_pc, U_n, U)

## pFibs block problem ##
problem = BlockProblem(J,F,U_n,aP=J_pc,bcs=bcs)  ## Uses custom schur ##
#problem = BlockProblem(J,F,U_n,bcs=bcs)         ## Use with default_params ##
...
## Override above parameters with this ##
default_params = {
    'ksp_view': True,
    'ksp_monitor_true_residual': True,
    'ksp_type': 'gmres',
...
## pFibs block solver #
solver = NonlinearBlockSolver(problem) ## Solve custom schur ##
#solver = NonlinearBlockSolver(problem,solver=default_params) ## Solve with default ##
